"""Acceptance gate for the local physics twin — spec sections 43, 65.

MuJoCo is not the authoritative twin -- that stays Isaac/OpenUSD on the
Spark (spec sections 52, 53). What this buys is that the object motion a
human sees in the client is produced by gravity and contact rather than by
a sine wave, on a machine with no Spark access.

These tests assert physics, not shape. A twin that merely emits
well-formed TwinState proves nothing.
...
from ar_contracts import TwinState  # noqa: E402
...
def test_is_deterministic():
    ...
    be reproduced (spec section 59 phase 12 determinism check)."""
