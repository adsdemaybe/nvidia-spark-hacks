"""A TwinState source backed by real rigid-body physics — spec section 43.

Same contract, same route, same client as the fixture stream
(tools/mock_twin_server.py) — what changes is that the numbers come out of
a solver instead of a sine wave. Objects fall, land, tip and settle because
of gravity and contact.

This is NOT the authoritative twin. OpenUSD in Isaac Sim on the Spark is
(spec sections 52, 53). What this is for: making the demo honest, and
making REPLAY mean something, on a machine that has neither.

Ported from Andrew's independent `arxr-sim` package during the arvr/arxr
consolidation (see ../../../STATE.md); imports adapted from `arxr.core` to
this repo's `ar_contracts`, otherwise unchanged.
"""
...
import numpy as np
from ar_contracts import ObjectState, RobotState, TaskState, TaskStatus, TwinState
...
# Matches tools/make_fixtures.py's BASE_TS_NS — an arbitrary fixed epoch so
# timestamps look real without a wall clock, shared across this repo's
# fixture generators.
EPOCH_NS = 1_700_000_000_000_000_000

# The cube counts as delivered once it is inside the bin's footprint and below
# its rim. Deterministic geometry, not a judgement call (spec section 63).
