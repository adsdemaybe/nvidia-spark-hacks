"""Scripted routines that drive the physics twin — spec section 43.

Honest about what these are: the *waypoints* are fixed. Nothing here plans
and nothing here is a learned policy. What is real is everything
downstream of the waypoints -- IK has to actually find a solution, the
actuators have to actually get there, and whether the cube ends up in the
bin is decided by contact and gravity rather than by the script declaring
victory.

This is a stand-in for a retargeted human demonstration, which is what
REPLAY will drive once TEACH episodes flow through the real pipeline
(spec section 13D — `ar_datapipe.retarget`).

Ported from Andrew's independent `arxr-sim` package during the arvr/arxr
consolidation (see ../../../STATE.md); unchanged except this docstring.
"""
