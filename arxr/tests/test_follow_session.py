"""Acceptance gate for FOLLOW session control (STRUCT_2.md 22, 64).

The gate that matters: STOP immediately halts target generation. A robot that
keeps receiving targets after the human hit stop is the failure this exists to
prevent.
"""
...
from arxr.core.schemas import DEFAULT_FOLLOW_DISTANCE_M, Pose
...
def test_a_stopped_session_stays_stopped_until_restarted():
    """Stop is not pause. Resuming a stopped follow has to be a deliberate act."""
    session = FollowSession()
    session.start()
    session.stop()
