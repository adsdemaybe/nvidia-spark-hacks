"""Acceptance gate for inverse kinematics (STRUCT_2.md 13D, 62).

This is what turns a human demonstration into robot joint targets. The gate in
62 is unambiguous: IK succeeds, joint limits hold, nothing is NaN, and an
unreachable pose is reported rather than approximated. A solver that quietly
returns its best effort for a pose outside the workspace produces training data
that claims the robot did something it cannot do.
"""
...
import pytest
from arxr.sim.ik import IKResult, solve_ik
...
def test_never_returns_nan(sim: MujocoTwinSource):
    ...
    NaN joints -- STRUCT_2.md 62 lists that explicitly."""
