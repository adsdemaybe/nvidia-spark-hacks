...
| Agent sandboxing | **OpenShell / NemoClaw** (NVIDIA, sandbox-base v0.0.90) — `setup/openshell/`. Non-interactive agent turns, snapshots, and a pluggable OpenAI-compatible inference endpoint pointed at our own vLLM. | Explicitly **not** a training framework: no train/tune/dataset/eval subcommand exists in either CLI. It is where reproducible *design and evaluation* batches run, and the right home for `freeform`'s model-written code |
| Docs RAG | **BUILT (2026-08-15), and not as planned.** `rag/` serves tscircuit + build123d on **:8220** — 2163 chunks, sub-millisecond search, no Postgres and no embedding model. pgvector and dense embeddings were both dropped deliberately: the queries are *identifier* lookups ("what props does pinheader take"), and a general sentence embedder is trained to put `pins` and `pinCount` near each other — the exact distinction that must not blur. BM25 with symbol, path-stem and prop-table boosts keeps them apart, costs no GPU on a box where memory is the scarce resource, and needs no service to run. A `dense` hook remains for conceptual paraphrase, where lexical is genuinely weaker. Corpus: tscircuit `llms.txt` + build123d `.rst` **and its docstrings**, since its API reference is generated from them. | Motivated by a measured failure, not a plan item: the model wrote `<connector pins="2">` wrapping `<connection>`, which is fluent, plausible and not tscircuit. Consumed asymmetrically — tscircuit goes into the PCB designer's prompt; build123d serves `text_to_cad` and `rag/ask.py`, because the CAD *design* agent names generators and needs no API docs |
...
| CAD kernel | build123d/OpenCascade; GLB via trimesh; collision hulls via CoACD. **`freeform` (2026-08-15) runs model-written build123d**, so a part nobody parameterised in advance can be described at all — until then the design agent's vocabulary was five generator names. Executes in a subprocess and returns as STEP: the realistic failure is a boolean that never terminates or an OCC segfault, both of which kill the parent in-process and are survivable in a child. | `geometry/inspect.py` sections a solid at both ends and counts holes, because bulk properties cannot see a wrong *feature*: asked for a 12mm spacer with a through bore, the model built one whose bore stops halfway, and bounding box (12×12×8) and mass (1.02 g) were both exactly right **for the solid that was built**. "It built and the mass is right" is not "it is the part" |
