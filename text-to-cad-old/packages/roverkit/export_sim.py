...
def write_urdf(links: list[Link], joints: list[Joint], path: str,
    ...
    # MuJoCo's URDF importer sets discardvisual=true by default, which silently
    # drops every <visual> mesh — the model then renders as bare collision
    # primitives and the STLs never load at all. This extension block is the
    # only way to keep them, and other consumers ignore unknown elements.
    mj = ET.SubElement(robot, "mujoco")
    ET.SubElement(mj, "compiler", discardvisual="false", strippath="false",
                  balanceinertia="false", fusestatic="false")
