...
DESIGN_VARS = {
    ...
    "BALLAST_M": (0.0, 1.2),
    # Yoke/link width. Bounded below by the 626 bearing seat
    # (BRG_OD + 2*WALL = 25); a NEMA23 face needs ~48 mm here.
    "LINK_W": (30.0, 72.0),
    # Gripper. Exposed so the critic can probe them — note that as of writing
    # NO criterion responds to either, which is exactly the blind spot that let
    # a 14 mm aperture ship on a robot rated for a 937 g payload.
    "RACK_TRAVEL": (10.0, 70.0),
    "JAW_LEN": (20.0, 80.0),
}
...
def reconfigure(**overrides) -> None:
    ...
    g["BASE_D"] = max(76.0, g["LINK_W"] * 2.2)  # yoke sits on the turntable
    g["PIN_D"] = g["BRG_ID"]
...
def check_mount_fit() -> list[str]:
    ...
    The CAD kernel will happily "cut" a bolt hole that falls outside the part —
    subtracting from empty space is a no-op — so an oversized motor yields a
    valid solid with no holes, and `cad_builds` passes. Three separate search
    routes selected a NEMA23 for a 34 mm yoke prong before this check existed;
    its bolt circle overhung by 6.6 mm and the design was unassemblable.
    """
    fails = []
    ...
    if need > half:
        fails.append(
            f"shoulder {SHOULDER_MOTOR} needs {need:.1f} mm of face on a "
            f"{half:.1f} mm prong (overhang {need - half:.1f} mm)")
    d = MOTORS[DRIVE_MOTOR]
    d_need = max(d["bolt_pitch"] / 2.0 + d["hole_d"] / 2.0, d["pilot_d"] / 2.0)
    if d_need > CHASSIS_H / 2.0:
        fails.append(
            f"drive {DRIVE_MOTOR} needs {d_need:.1f} mm on a "
            f"{CHASSIS_H / 2.0:.1f} mm side wall")
    return fails
